export function stickyHeader(){ //-----------------------------
 ...
 export function burgerMenu() {
	const $msBurgerBtn = document.querySelectorAll('.burger__icon'),
       	$body = document.body;
	      // $targetActive = document.querySelector('.navig');

				if ($msBurgerBtn.length > 0) {
					for (let i = 0; i < $msBurgerBtn.length; i++){

						const $el = $msBurgerBtn[i];
						$el.addEventListener('click', function(e){
							$body.classList.toggle("--js-lock__body--burgerMenu");
							e.preventDefault();      // запрет перезагрузки
						});
					}
	    }	// if	// f_checkMobile();  // f_linkGoTo(); // плавная прокрутка
}// -----------  END  burgerMenu ------------------------
