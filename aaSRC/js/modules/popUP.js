...
export function popUP() { 
 ...
 const timeout = 0;  // 800 милесекунд = транзишн 0.8s
 const popupCloseIcon = document.querySelectorAll('.--js-popup_close'); // в модл окне объект с классом .close-popUP
 ...
 if (popupCloseIcon.length > 0) {
   for (let i = 0; i < popupCloseIcon.length; i++){
     ...
     el.addEventListener('click', function(e){
       popupClose(el.closest('.--js-popup')); // цикл будет искать родителя с класом .popup и его будет закрывать
       e.preventDefault();      // запрет перезагрузки
     });
   }
 }// END if
...
function popupOpen(curentPopup){ // функция открытия модл окна
    if (curentPopup && unlock) { // проверка окна + переменной unlock
     const popupActive = document.querySelector('.--js-popup.--js_open-pop-up');
     ...
     curentPopup.classList.add("--js_open-pop-up"); // добавить класс -JS

    
     //  let $popupLink = curentPopup.querySelector('.-js_focus');
    //  if ($popupLink){ $popupLink.focus(); } 

     //  alert("ПРОВЕРКА popupActive " + curentPopup.innerText); 

     // curentPopup.addEventListener('click', function(e){ //e   закрыть по клику на фон
...
function popupClose(popupActive, doUnLock = true) { //  doUnLock = не блочить скрол 2 раза
   if(unlock){
     popupActive.classList.remove('--js_open-pop-up');
     if(doUnLock){
